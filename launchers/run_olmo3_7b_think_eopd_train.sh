#!/bin/bash
# Olmo-3-7B-Think + EOPD baseline (method=eopd): paper-faithful EOPD
# implementation (entropy-conditioned per-token reverse-KL + top-k forward-KL).
# Output: outputs/olmo3_7b_think_eopd/ (LoRA adapter)
#         outputs/olmo3_7b_think_eopd_merged/ (merged full model)
#
...
train_and_merge "olmo3_7b_think_eopd" \
    --method eopd
