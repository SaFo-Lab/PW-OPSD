#!/bin/bash
# Qwen3-4B base + OPSD baseline (method=opsd): train LoRA adapter and merge
# into a full HuggingFace-loadable model directory. Touches a .ready_for_eval
# sentinel under outputs/ that the eval launcher can detect.
set -Eeuo pipefail
...
# Validate that a merged model directory looks complete: config.json plus at
# least one safetensors shard, and tokenizer files. Returns 0 if valid.
validate_merged_dir() {
  local d="$1"
  [ -f "$d/config.json" ] || return 1
  # at least one safetensors shard with non-zero size
  local n_shards
  n_shards=$(find "$d" -maxdepth 1 -name "*.safetensors" -size +0 -printf 1 2>/dev/null | wc -c)
  [ "$n_shards" -ge 1 ] || return 1
  # tokenizer presence (any of these is enough)
...
train_and_merge() {
  ...
  accelerate launch --config_file "$REPO_ROOT/code/accelerate_4gpu.yaml" \
    --num_processes 4 --gradient_accumulation_steps 2 --main_process_port $PORT \
    ...
    --vllm_gpu_memory_utilization 0.4 --vllm_tensor_parallel_size 1 \
...
# OPSD baseline (uniform forward-KL): method=opsd dispatches to the
# unmodified OPSD path inside ea_opsd_trainer.py.
train_and_merge "qwen3_4b_base_opsd" --method opsd

echo "[$(date)] OPSD baseline TRAIN+MERGE COMPLETE"
