#!/bin/bash
# Qwen3-4B base + PW-OPSD Moderate (method=pwopsd) train-only.
#   Output: outputs/qwen3_4b_base_pwopsd/ (LoRA adapter)
#           outputs/qwen3_4b_base_pwopsd_merged/ (merged full model)
#
# PW-OPSD schedule: (w_min, tau, s) = (0.25, 0.30, 0.10), the
# diagnostic-derived "Moderate" configuration from the paper. The
# ea_opsd_trainer.py PWOPSD branch computes the position weight using
# per-sequence response length (not batch-padded max length) and reduces
# the loss by sequence-level mean (not global token mean).
set -Eeuo pipefail
...
train_and_merge "qwen3_4b_base_pwopsd" \
    --method pwopsd \
    --position_w_min 0.25 \
    --position_tau 0.30 \
    --position_s 0.10

echo "[$(date)] PW-OPSD TRAIN+MERGE COMPLETE"
