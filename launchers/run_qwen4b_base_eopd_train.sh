#!/bin/bash
# Qwen3-4B base + EOPD baseline (method=eopd): paper-faithful EOPD
# implementation (entropy-conditioned per-token reverse-KL + top-k forward-KL).
# Output: outputs/qwen3_4b_base_eopd/ (LoRA adapter)
#         outputs/qwen3_4b_base_eopd_merged/ (merged full model)
#
...
train_and_merge "qwen3_4b_base_eopd" \
    --method eopd
