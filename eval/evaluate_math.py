...
def grade_answer(predicted, ground_truth) -> bool:
    ...
    Args:
        predicted: The predicted answer (already extracted from \\boxed{}).
            May be str, int, float, or None.
        ground_truth: The ground truth answer (any type that str() can handle).
    ...
    if predicted is None:
        return False
    # Defensive: coerce to str so non-string inputs (e.g. numeric AIME answers)
    # cannot crash the parser-exception fallback path that calls .replace().
...
def evaluate_math500(
    ...
    if dataset_name.lower() == "math500":
        ...
        hmmt_path = os.environ.get("HMMT25_PARQUET_PATH", "./data/hmmt25_clean/train.parquet")
        dataset = load_dataset("parquet", data_files=hmmt_path, split="train")
        print(f"Loaded MathArena/hmmt_feb_2025 dataset (from local clean parquet at {hmmt_path}) with {len(dataset)} problems")
    else:
        raise ValueError(
    ...
    for example in dataset:
        ...
        if dataset_name.lower() == "amo-bench":
            ...
            # MATH500 format. Prefer the dataset's canonical 'answer' field;
            # only fall back to extracting from 'solution' if 'answer' is
            # absent (legacy datasets). The previous behavior of always
            # re-parsing 'solution' could yield gt_answer = full solution
            # text whenever the boxed-answer extractor failed.
            problem = example["problem"]
            question_id = None
            if 'answer' in example and example['answer'] is not None and str(example['answer']).strip():
                gt_answer = str(example['answer'])
            else:
    ...
    for idx, (output, problem, gt_answer, question_id) in enumerate(
        ...
        # Majority vote: cluster ALL N predictions by math equivalence so that
        # unformatted (no \boxed{}) generations participate as a distinct
        # "INVALID" cluster and cannot be silently dropped before the vote.
        # Without this, e.g. 11/12 unformatted + 1 formatted-correct used to be
        # reported as majority-correct.
        majority_vote_correct = False
        clusters = []  # list of [representative_or_None_for_invalid, count]
        for pred, fmt in zip(predicted_answers, is_formatted_list):
            if not fmt:
                # all unformatted predictions collapse into one INVALID cluster
            ...
            for c in clusters:
                if c[0] is None:
                    continue  # never merge a real prediction into INVALID
                try:
                    # grade_answer may not be symmetric (it parses arg2 as the
                    # ground truth and arg1 as the prediction), so check both
                    # orderings to make the equivalence relation symmetric.
        ...
        if clusters:
            ...
            most_common_answer = clusters[0][0]
            if most_common_answer is None:
                majority_vote_correct = False  # INVALID cluster wins
            else:
                try:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Random seed for vLLM SamplingParams (default: None -> vLLM default randomness). "
             "Set explicitly for multi-seed evaluation reproducibility.",
    )
