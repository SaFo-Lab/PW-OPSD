#!/bin/bash
# Qwen3-4B base + REOPOLD baseline (method=reopold) train-only.
#   Output: outputs/qwen3_4b_base_reopold/ (LoRA adapter)
#           outputs/qwen3_4b_base_reopold_merged/ (merged full model)
#
# REOPOLD hyperparameters: lambda=0.1, beta=0.2, T_switch=50.
# The reward R = log pi_T(o_t) - log pi_theta(o_t) is computed on the full
# vocabulary log_softmax (no top-k / top-p truncation), matching Eq 7-8 of
# the REOPOLD paper. The shared training chassis is identical to the other
# launchers (max_steps=150, max_completion_length=1024, lr=5e-6,
# per-device-batch=4, gradient-accumulation=2 on 4 GPUs).
...
train_and_merge "qwen3_4b_base_reopold" \
    --method reopold \
    --reopold_lambda 0.1 \
    --reopold_beta 0.2 \
    --reopold_t_switch 50

echo "[$(date)] REOPOLD TRAIN+MERGE COMPLETE"
