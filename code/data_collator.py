...
class SelfDistillationDataCollator:
    ...
    def __call__(self, features):
        ...
        for feature in features:
            ...
            # Some chat templates (e.g. DeepSeek-R1-Distill-*) ignore the
            # enable_thinking flag and unconditionally append an open <think>
            # tag at the end of the rendered prompt, leaving the model in
            # "thinking" mode. We detect this by checking whether the prompt
            # ENDS with an unclosed <think> tag (suffix-scoped to avoid false
            # positives from <think> appearing inside the user's problem text).
            # If so, append </think> to replicate enable_thinking=False
            # semantics on tokenizers that ignore that flag.
            if re.search(r"<think\s*>\s*$", student_prompt):
                student_prompt = student_prompt + "</think>\n\n"
