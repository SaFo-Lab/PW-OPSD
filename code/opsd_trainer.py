    ...
    def __init__(
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                # enforce_eager=True disables CUDA graph capture; required for some
                # base+LoRA model combinations (e.g. Phi-3/Phi-4-mini) where vLLM's
                # LoRA logits processor crashes during graph capture with
                # "index out of bounds" in IndexKernel.cu. Costs ~10-20% throughput.
                _enforce_eager = bool(int(os.environ.get("VLLM_ENFORCE_EAGER", "0")))
